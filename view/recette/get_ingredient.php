...
if (isset($_GET['term'])){
    ...
    $chaine =  htmlspecialchars($_GET['term']) ;

    try {
        $conn = new PDO("mysql:host=".DB_SERVER.";dbname=".DB_NAME, DB_USER, DB_PASSWORD);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $stmt = $conn->prepare('SELECT nom_ingredient FROM ingredients WHERE ingredients.nom_ingredient LIKE :term ORDER BY nom_ingredient ASC LIMIT 7 ' );
        $stmt->execute(array('term' => $chaine.'%'));

        while($row = $stmt->fetch()) {
            $return_arr[] =  $row['nom_ingredient'];
        }
